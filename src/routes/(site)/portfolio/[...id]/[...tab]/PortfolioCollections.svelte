<script lang="ts">
    ...
    // Add view selection
    let viewMode: 'grid' | 'pie' | 'mosaic' | 'list' = 'grid';
    ...
    $: pieChartData = Array.from(new Set(tokens.map((t) => t.contractId)))
        .map((contractId) => {
            ...
            return {
                name: collection?.highforgeData?.title || `Collection #${contractId}`,
                value: collectionTokens.length
            };
        })
...
<div class="px-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                        <button 
                            class="px-3 py-1 rounded-lg text-sm font-medium transition-colors
                                   {viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}"
                        ...
                        <button 
                            class="px-3 py-1 rounded-lg text-sm font-medium transition-colors
                                   {viewMode === 'pie' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}"
                        ...
                        <button 
                            class="px-3 py-1 rounded-lg text-sm font-medium transition-colors
                                   {viewMode === 'mosaic' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}"
                            on:click={() => viewMode = 'mosaic'}
                        >
                            <i class="fas fa-images mr-2"></i>Mosaic
                        </button>
                        <button 
                            class="px-3 py-1 rounded-lg text-sm font-medium transition-colors
                                   {viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}"
        ...
        <div class="p-6">
            {#if viewMode === 'grid'}
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-auto grid-flow-dense" transition:fade>
            ...
            {:else if viewMode === 'pie'}
                <div class="flex justify-center items-center min-h-[400px]" transition:fade>
                    <PieChart data={pieChartData} {collections} />
                </div>
            {:else if viewMode === 'mosaic'}
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2" transition:fade>
                    {#each Array.from(new Set(tokens.map((t) => t.contractId)))
                        .map((contractId) => {
                            const collection = collections.find(c => c.contractId === contractId);
                            const collectionTokens = tokens.filter((t) => t.contractId === contractId && t.owner === walletId);
                        ...
                        {@const collection = item.collection}
                        {@const tokens = item.tokens}
                        {#if collection && collectionMatchesSearch(collection)}
                            <div class="group relative aspect-square rounded-lg overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300"
                                 on:click={() => toggleCollection(collection.contractId)}>
                                {#if tokens[0]?.metadata?.image}
                                    <img 
                                        src={getImageUrl(tokens[0].metadata.image, 400)} 
                                        alt={collection.highforgeData?.title || `Collection #${collection.contractId}`}
            ...
            {:else}
                <div class="space-y-4" transition:fade>
                    {#each Array.from(new Set(tokens.map((t) => t.contractId)))
                        ...
                        {@const tokens = item.tokens}
                        {#if collection && collectionMatchesSearch(collection)}
                            <div class="flex items-center gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
                                {#if tokens[0]?.metadata?.image}
                                    <img 
                                        src={getImageUrl(tokens[0].metadata.image, 100)} 
                                        alt={collection.highforgeData?.title || `Collection #${collection.contractId}`}
                                        class="w-16 h-16 rounded-lg object-cover"
                                    />
                                {:else}
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        {tokens.length} NFTs owned
                                    </p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <a href={`/collection/${collection.contractId}`}
                                       class="px-3 py-1 bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-800/50 text-blue-700 dark:text-blue-300 rounded-lg transition-colors">
                                        View
                                    </a>
                                    <button 
                                        on:click={() => toggleCollection(collection.contractId)}
                                        class="px-3 py-1 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900/30 dark:hover:bg-purple-800/50 text-purple-700 dark:text-purple-300 rounded-lg transition-colors">
                                        <i class="fas {expandedCollections.has(String(collection.contractId)) ? 'fa-chevron-up' : 'fa-chevron-down'}"></i>
                                    </button>
                                </div>
                            </div>
                            <!-- Expanded Collection Content for List View -->
                            {#if expandedCollections.has(String(collection.contractId))}
